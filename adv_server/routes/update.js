...
router.put('/status', async (req, res) => {
  const { secret_hash, src_escrow_status, dst_order_status } = req.body;
  const updates= {};
  if (src_escrow_status) updates.src_escrow_status = src_escrow_status;
  if (dst_order_status) updates.dst_order_status = dst_order_status;
...
router.put('/src_escrow', async (req,res) => {
  ...
  updates.src_escrow_status = "deployed_locked";
  const { data, error } = await supabase.from("Adv_Relayer_Data").update(updates).eq('secret_hash', secret_hash);
