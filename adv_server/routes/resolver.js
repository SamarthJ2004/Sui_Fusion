...
router.get("/", async (req,res) => {
    ...
    let {data, error} = await supabase.from("Adv_Relayer_Data").select("*").eq("secret_hash", secret_hash).single();
...
router.get("/redeem", async(req, res) => {
    ...
    await supabase.from("Adv_Relayer_Data").delete().eq("secret_hash", secret_hash);
